...
class Model:
    def __init__(self, pipe, model, tokenizer):
        self.pipe = pipe
        self.model = model
    ...
    def output(self, text):
        ...
        chunks = []
        for doc in docs:
            name = re.search(r'"([^"]+)"', doc.page_content)
            module = name.group(1) if name else "IndependentChunk"
            if module not in modules and module != "IndependentChunk":
                modules.append(module)

            chunks.append({"module": module, "chunk": doc.page_content, "score": doc.metadata["score"]})
...
"""
        ...
        return {"output": answer, "modules": modules, "saliency": saliencyHTML}
...
try:
    ...
    print("Error while loading generative model:", e)
...
def loadChat():
    ...
    with open("KAL/outputs.txt", "r", encoding="utf-8") as f:
        ...
        for entry in data:
            messages.append({"from": "user", "text": entry["query"], "show": False, "more": False, "saliency": "", "modules": []})
            messages.append({"from": "bot", "text": entry["response"], "show": False, "more": True, "saliency": entry["saliency"], "modules": entry["modules"]})

    # print(messages)
    return flask.make_response(flask.jsonify(messages))
