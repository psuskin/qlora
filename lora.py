...
class LoraLayer:
    counter = 0
    weights_A_64 = torch.load(REPLACE_THIS, map_location=torch.device('cpu'))
    keys_A_64 = [key for key in weights_A_64 if "lora_A" in key]
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            r = self.lora_A[adapter_name].weight.size(dim=0)
            #print(LoraLayer.counter)
            if r != 64:
                # Below doesn't create identical tensor
                #weight_A_64 = torch.empty((64, self.in_features), dtype=self.weight.dtype, device=self.weight.device)
                #nn.init.kaiming_uniform_(weight_A_64, a=math.sqrt(5))
