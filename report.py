...
def generate(model, tokenizer, template, prompt, max_new_tokens=2000):
    input = template.format(prompt=prompt[0], context=prompt[1])
...
if __name__ == "__main__":
    ...
    implementations = [
        ...
        ('KARG', 'meta-llama/Llama-2-13b-hf', 'output/klio-autoregressive-2-13b-r64-noeval/checkpoint-1875/adapter_model', "### Human: {prompt}\nContext: {context}### Assistant: ")
    ]
    ...
    for implementation in implementations:
        ...
        if set([prompt[0] for prompt in prompts]) == set([response["query"][0] for response in responses[name]]):
            continue

        model, tokenizer = load_model(implementation[1], implementation[2])
        for prompt in prompts:
            if prompt[0] in [response["query"][0] for response in responses[name]]:
                continue
